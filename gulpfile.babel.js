...
var zipName = 'hurdlclient.zip';
var bucketName = 'falcon-frontend'
var awsRegion = process.env.GULP_AWS_REGION || 'us-east-1';
var applicationName = 'hurdl-client';
var environmentName = 'hurdl-client-' + (process.env.environment || 'local');
...
AWS.config.update({
    accessKeyId: process.env.AMAZON_ACCESS_KEY_ID || undefined,
    secretAccessKey: process.env.AMAZON_SECRET_ACCESS_KEY || undefined
});
...
gulp.task('zip-frontend', function(cb) {
...
gulp.task('copy-node-package', function () {
    return gulp.src('./package.json')
        .pipe(gulp.dest('./dist'));
});

gulp.task('copy-bower-package', function () {
    return gulp.src('./bower.json')
        .pipe(gulp.dest('./dist'));
});

gulp.task('deploy', ['update-elastic-beanstalk']);

gulp.task('update-elastic-beanstalk', ['push-to-s3'], function(done) {
    var eb = new AWS.ElasticBeanstalk({
        region: awsRegion
    });
    eb.createApplicationVersion({
        ApplicationName: applicationName,
        VersionLabel: process.env.APPVEYOR_BUILD_VERSION || '0.0.0',
        SourceBundle: {
            S3Bucket: bucketName,
            S3Key: zipName
        }
...
gulp.task('push-to-s3', function(done) {
    var s3 = new AWS.S3();
    ...
    s3.createBucket(bucketConfiguration, function(err) {
